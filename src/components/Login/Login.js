...
const Login = () => {
    ...
    const handleGgLogin = async ()=>{
        const {user,_tokenResponse}= await signInWithPopup(auth, ggProvider);
          if(_tokenResponse.isNewUser){
              addDocument('users',{
                  displayName: user.displayName,
                  keywords: generateKeywords(user.displayName),
        ...
        <div className='box'>
            <div className='content__login'>
                ...
                <div className='content__buttons '>
                    <button onClick={handleGgLogin} className='content__button content__button__google' style={{border:0}}><GoogleCircleFilled style={{color:'white', marginRight:'10px', fontSize:'1rem'}}/><span style={{color:'white'}}>Continue with <span style={{fontWeight:'800'}}>Google</span></span></button>
                </div>
            </div>
        </div>
    )
